...
import torch
if model_type in ["fused", "pfused"]:
    for batch, (x_omic, x_img, e, t) in enumerate(dataloader):
        x_omic, x_img, e, t = x_omic.to(device), x_img.to(device), e.to(device), t.to(device)
        pred = model(x_omic, x_img)
        # ...loss/backward...
else:
