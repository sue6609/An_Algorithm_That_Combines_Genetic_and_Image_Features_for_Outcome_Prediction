...
def parse_args():
    ...
    parser.add_argument("--output_size", type=int, default=1, help="單模態模型輸出維度")
    ...
    parser.add_argument("--output_dim", type=int, default=8, help="融合模型輸出維度")
...
if __name__ == "__main__":
    ...
    full_val_dataset = DatasetLoader(args.data_cv, 'test', args.mode)
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=mixed_collate
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=mixed_collate
    )
    # Model
...
torch.save(best_model_state, f"models/{args.model}_{args.epochs}epochs_{timestamp}.pth")

    torch.save(best_model_state, f"models/{args.model}_{args.epochs}epochs.pth")
    ...
    # Plot
    plot_metrics(train_losses, test_losses, train_cindexs, c_indices, args.epochs, args.model)
