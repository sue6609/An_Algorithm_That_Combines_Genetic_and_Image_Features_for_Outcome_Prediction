...
class SNN_Model(nn.Module):
    def __init__(self, input_size, output_size, dropout_rate):
        ...
        self.classifier = nn.Sequential(nn.Dropout(0.3), nn.Linear(16, output_size))
    ...
    def forward(self, **kwargs):
        ...
        x = self.encoder(x)
        x = self.classifier(x)
        return x * self.output_range + self.output_shift
...
class PSNN_Model(nn.Module):
    ...
    def forward(self, **kwargs):
        self.fc1.weight.data = self.fc1.weight.data.mul(self.pathway_mask)
        x = kwargs['x_omic']
        x = self.fc1(x)
        x = self.encoder(x)
        x = self.classifier(x)
...
class CNN_SNN_Model(nn.Module):
    def __init__(self, input_size, output_size, dropout_rate): # 帶改
...
class FUSED_Model(nn.Module):
    def __init__(self, input_size, output_size, dim_1, dim_2, output_dim, dropout_rate):
        super(FUSED_Model, self).__init__()
        self.model_a = CNN_SNN_Model(input_size, output_size, dropout_rate)  # 模型A
        self.model_b = GatedMultimodalUnit(dim_1, dim_2, output_dim)  # 模型B

    def forward(self, **kwargs):
        gene_data = kwargs['x_omic']
        images_data = kwargs['x_path']
...
class CNN_PSNN_Model(nn.Module):
    def __init__(self,device ,input_size, output_size, pathway_nodes, pathway_mask, dropout_rate): # 帶改
...
class PFUSED_Model(nn.Module):
    def __init__(self, input_size, output_size, dim_1, dim_2, output_dim, pathway_nodes, pathway_mask, dropout_rate):
        super(PFUSED_Model, self).__init__()
        self.model_a = CNN_PSNN_Model(input_size, output_size, pathway_nodes, pathway_mask, dropout_rate)  # 模型A
        self.model_b = GatedMultimodalUnit(dim_1, dim_2, output_dim)  # 模型B
