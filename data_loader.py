...
class DatasetLoader():
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["path"]:
            ...
            return (0, self.transforms(img), single_e, single_t)
        elif self.mode in ["omic"]:
            return (torch.tensor(self.X_omic[index]).float(), 0, single_e, single_t)
        elif self.mode == "pathomic":
            img = Image.open(self.X_path[index]).convert('RGB')
            return (torch.tensor(self.X_omic[index]).float(), self.transforms(img), single_e, single_t)
